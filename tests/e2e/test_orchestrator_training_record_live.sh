...
has_chat_messages=$(echo "$op_resp" | jq '.[0].affected_tables | index("chat_messages") != null')
if [ "$has_chat_messages" != "true" ]; then
  echo "training record operation_logs missing chat_messages in affected_tables" >&2
  echo "$op_resp" >&2
  exit 1
fi
