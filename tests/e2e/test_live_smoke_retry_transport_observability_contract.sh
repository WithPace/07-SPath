...
# Scenario A: one transport retry then success should emit transport retry log.
reset_uid_counter
reset_curl_counter
request_ids=()
curl_fail_until=1
...
trap 'rm -f "$uid_counter_file" "$curl_counter_file" "$scenario_a_log" "${scenario_b_log:-}"' EXIT

ORCH_MAX_ATTEMPTS=4 ORCH_RETRY_BASE_DELAY_SECONDS=1 orchestrator_call_with_retry "training" "transport_retry_log" "0" >/dev/null 2>"$scenario_a_log"

grep -q 'orchestrator retry: module=training request_id=req-1 attempt=1/4 sleep_seconds=1 reason=transport_error' "$scenario_a_log" || fail "scenario_a retry log mismatch"

# Scenario B: transport exhausted should emit transport terminal log.
reset_uid_counter
reset_curl_counter
request_ids=()
curl_fail_until=0
...
grep -q 'orchestrator terminal_failure: module=training request_id=req-3 attempt=3/3 reason=transport_error_exhausted' "$scenario_b_log" || fail "scenario_b terminal log mismatch"

echo "live smoke retry transport observability contract present"
